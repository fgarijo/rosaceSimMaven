...
/**
 * Implementacion por defecto de las acciones que se ejecutaran por parte del
 * gestor de recursos.
 *
 * @author
 * @created
        ...
        try {
            ...
            if (lista.isEmpty()) {
                ...
                trazas.aceptaNuevaTraza(new InfoTraza(
                        ...
                        InfoTraza.NivelTraza.debug));
                 this.informaraMiAutomata("listaRecursosGestionadosVacia");
            }else {
        ...
        while (iterador.hasNext() && (encontrado == false)) {
            ...
            if (j == indice.intValue()) {// es el que debemos arrancar
                ...
                try {
                    ...
                    // si todo ha ido bien, debemos anydirlo a la lista de
                    // objetos gestionados por el gestor
                    logger.debug("GestorRecursos: Anyadiendo recurso " + nombre + " a la lista de objetos gestionados.");
                    trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
                            "Anyadiendo recurso " + nombre + " a la lista de objetos gestionados.",
                            InfoTraza.NivelTraza.debug));
...
//			String esteNodo = descGestorRecursos.getNodo().getNombreUso();
    ...
    private void crearUnRecursoEnNodoRemotoConRMI(DescInstanciaRecursoAplicacion recurso) throws Exception {
        ...
        if (!misInterfacesEstanEnElRegistroRMILocal) { //Las anyado
            ...
            if (!AdaptadorRegRMI.addElement2LocalRegRMI(NombresPredefinidos.ITF_USO + nombreAgente, this.itfUsoPropiadeEsteAgente)) { // No se ha podido anyadir la interfaz de uso al registro local
                trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
                        "Error al anyadir mi interfaz al registro RMI local. " + nombreAgente + "No se ha podido anyadir  la itf de uso del Gestor de nodo en el registro RMI local " + identNodoRecurso,
                        InfoTraza.NivelTraza.error));
            }
        }
        try {
...
//                    ItfUsoAgenteReactivo itfUsoGestorNodo = (ItfUsoAgenteReactivo) AdaptadorRegRMI.getItfAgenteRemoto(NombresPredefinidos.NOMBRE_GESTOR_NODO, NombresPredefinidos.ITF_USO);
                    if (itfUsoGestorNodo != null) {
                        ...
                        //  Deberiamos esperar una confirmacion de la creacion.
    ...
    public void reintentarCreacionRecurso(List<DescInstanciaRecursoAplicacion> lista, DescInstanciaRecursoAplicacion recurso,
        ...
        if (!fin_reintentos) { // se reintenta la creacion
            ...
            try {
                ...
                logger.debug("GestorRecursos: Anyadiendo recurso " + idRecurso + " a la lista de objetos gestionados.");
                trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
                        "Anyadiendo recurso " + idRecurso + " a la lista de objetos gestionados.",
                        InfoTraza.NivelTraza.debug));
...
//						NombresPredefinidos.NOMBRE_GESTOR_RECURSOS));
	 ...
	 * encontro ningun recurso con nombre "+nombreRecurso+" dentro de la lista
	 * de objetos gestionados."); throw new NullPointerException(); }
     */
    /**
     * Decide que hacer en caso de fallos en los recursos.
     */
...
//						NombresPredefinidos.NOMBRE_GESTOR_RECURSOS));
    ...
    /**
     * Da orden de terminacion a todos los recursos que se encuentran
     * activos/arrancando
     */
    public void terminarRecursosActivos() {
        ...
        while (enumRecursos.hasMoreElements()) {
            try {
                ...
                logger.debug("GestorRecursos: Recuperando Itf Gestion del recurso " + nombre + ".");
                trazas.aceptaNuevaTraza(new InfoTraza("GestorRecursos",
