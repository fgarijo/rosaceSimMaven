...
//                this.arranca();
    ...
    public synchronized void procesarInfoControlAgteReactivo(Object infoParaProcesar) {
        ...
        String infoTrazar = "Automata en el estado : " + this.getEstadoControlAgenteReactivo() + "  Se procesa el  input : " + infoParaProcesar;

//        NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte, infoTrazar, InfoTraza.NivelTraza.debug);
...
//                      + " Cambiar el contenido del evento");
        ...
        if (!resultadoProcesoInput) infoTrazar = infoTrazar + "No hay transicion asociada al input:  " + infoParaProcesar;
        else  infoTrazar =infoTrazar + " \n Se ha ejecutado  la accion :  " + this.getAccionEjecutada();
        NombresPredefinidos.RECURSO_TRAZAS_OBJ.trazar(identAgte, infoTrazar, InfoTraza.NivelTraza.debug);
    }
...
//               numParametros = 1;
    ...
    public synchronized String getAccionEjecutada() {
        return automataControl.getAccionEjecutada();
    }
...
//    }
     ...
     *
     * El gestionador obtendr las interfaces del gestor a partir del repositorio
     * de interfaces y podr validar la informacin.
     *
     */
}
