...
// Se crea la percepcion y se le pasa la interfaz del control
	...
	//	AgenteReactivoAbstracto patron = new AgenteReactivoImp(nombreInstanciaAgente,itfControlAgteReactivo,itfProductorPercepcion,itfConsumidorPercepcion);
        ...
        // paso 3. 4  Se define la interfaz de uso del agente creado en las acciones sem??nticas espec?�ficas
...
//     }
   ...
   //   String rutaBusqueda = NombresPredefinidos.RUTA_SRC+rutaComportamiento;
        ...
        rutaComportamiento = rutaComportamiento.replace("icaro.","");
        rutaComportamiento = "/" +rutaComportamiento.replace(".", "/");
         String rutaAutomata = rutaComportamiento +"/"+ NombresPredefinidos.FICHERO_AUTOMATA;
         InputStream input = this.getClass().getClassLoader().getResourceAsStream(rutaAutomata);
...
//                    rutaAutomata = "iniciador/automataIniciador.xml";
                    input = this.getClass().getResourceAsStream(rutaAutomata);
                    if  (input != null) return rutaAutomata;
                    else {
                        rutaAutomata = "gestores/iniciador"+"/automata"+nombreEntidad+".xml";
//                        File file1 = new File(classLoader.getResource("gestores/iniciador"+"/automata"+nombreEntidad+".xml").getFile()) ;
//                        InputStream input2 = classLoader.getResourceAsStream("gestores/iniciador"+"/automata"+nombreEntidad+".xml");
//                         File resourcesDirectory = new File("gestores/iniciador"+"/automata"+nombreEntidad+".xml");
//                     String[] ficherosRuta= file1.list();
//                         String rutaAbsoluta=    resourcesDirectory.getAbsolutePath();
//                     if(!rutaAbsoluta.isEmpty())System.out.println(rutaAbsoluta);
                     input = this.getClass().getClassLoader().getResourceAsStream(rutaAutomata);
//                     rutaAutomata=rutaAbsoluta;
                    }
                    if  (input != null) return rutaAutomata;
                    else {
