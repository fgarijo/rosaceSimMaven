package org.icaro.gestores.iniciador;
...
import org.icaro.infraestructura.entidadesBasicas.descEntidadesOrganizacion.ComprobadorRutasEntidades;
...
import org.icaro.infraestructura.recursosOrganizacion.configuracion.imp.ClaseGeneradoraConfiguracion;
...
public void crearRecursosNucleoOrganizacion () throws Exception {
    ...
    try {
   // Se crea el recurso de configuracion y posteriormente se debería dar orden al recurso
...
//            DescComportamientoAgente desCompGestor = configuracionExterna.getDescComportamientoAgente(identGestor);
         ...
         if ( identGestor.startsWith(NombresPredefinidos.NOMBRE_GESTOR_ORGANIZACION)){
                if (locComportamientoGestor.isEmpty())
                        locComportamientoGestor = NombresPredefinidos.RUTA_GESTORES;
