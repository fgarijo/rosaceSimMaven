...
public class InfoCasoSimulacion {
    ...
    public InfoCasoSimulacion(@Element(name = "identCaso") String casoId, @Element(name = "identEscenario") String escenarioId) {
        ...
        infoRescateVictimas = new HashMap<>();
        conjVictimasRescatadas = new TreeSet<>();
        infoRobotVictimasSalvadas = new HashMap<>();
        infoRobotVictimasAsignadas = new HashMap<>();
...
//       InfoRescateVictima infoVictima = infoRescateVictimas.get(idVictima);
            if (!infoRobotVictimasAsignadas.containsKey(idRobot)) {
                ...
                infoRobotVictimasAsignadas.get(idRobot).add(idVictima);
            }
            if (!infoRescateVictimas.containsKey(idVictima)) {
                numeroVictimasAsignadas++;
            }
    ...
    public synchronized void addInfoRescateVictima(InfoRescateVictima infoRescate) {
        ...
        if (idVictima != null) {
            infoRescateVictimas.put(idVictima, infoRescate);
        }
        ...
        if (!infoRobotVictimasSalvadas.containsKey(idRobot)) {
            ...
            victimasRescatadasRobot = infoRobotVictimasSalvadas.get(idRobot);
        }
        victimasRescatadasRobot.addVictima(idVictima);
        conjVictimasRescatadas.add(infoRescate);
        if (idVictima.equals(conjVictimasRescatadas.last().getvictimaId())) {
            identsVictimasRescatadas.addVictima(conjVictimasRescatadas.last().getvictimaId());
        } else {
            reordenarIdsVictimasRescatadas();
        }
...
//   @XmlElement (name = "victimasRescatadas")
    ...
    public ArrayList<PuntoEstadistica> getSerieAsignacionVictimas() {
        ArrayList<PuntoEstadistica> serieAsignacionVictimas = new ArrayList<>();
    ...
    public ArrayList<PuntoEstadistica> getSerieDatosParaVisualizar(String serieDatos) {
        ArrayList<PuntoEstadistica> serieRescateVictimas = new ArrayList<>();
        int numVictimas = 0;
        ...
        while (iter.hasNext()) {
            infoRescVict = iter.next();
            numVictimas++;
            if (serieDatos.equalsIgnoreCase(SerieDatosTiempoPeticion)) {
                tiempoReportado = infoRescVict.getTiempoPeticion();
            } else if (serieDatos.equalsIgnoreCase(SerieDatosTiempoAsignacion)) {
                tiempoReportado = infoRescVict.getTiempoAsignacion();
            } else if (serieDatos.equalsIgnoreCase(SerieDatosTiempoRescate)) {
