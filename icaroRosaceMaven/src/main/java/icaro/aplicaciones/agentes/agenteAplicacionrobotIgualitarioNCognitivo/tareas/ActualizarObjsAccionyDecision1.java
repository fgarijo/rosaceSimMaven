...
/**
 *
 * @author FGarijo rule Foc4: Focalizacion en una nueva decision e inicio de
 * ayuda a victima : victimaCC:Victim(idVict:name) estatusRobot
 * :RobotStatus1(bloqueado==false) objAcc :AyudarVictima(state ==
 * Objetivo.PENDING, victimId == idVict) objDec: DecidirQuienVa (state ==
 * Objetivo.SOLVED,objectDecisionId == idVict ) focoActual:Focus(foco == objDec)
 * infoDecision: InfoParaDecidirQuienVa(idElementoDecision == idVict)
 * tarea.ejecutar(objetivosAccion,objetivosDecision,
 * objAcc,infoDecision,focoActual, estatusRobot, victimaCC,victimsArescatar);
 */
