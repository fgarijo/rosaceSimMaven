...
//    private boolean editando;
    ...
    private LinkedBlockingDeque<Object> colaCoordRobot;
    private VisualizacionPosRobot visualizarPosicionRobot;
    private boolean visualizacionPosicionesIniciada;
    private Object[] posRobot;
    ...
    public ControladorVisualizacionSimulRosace(NotificadorInfoUsuarioSimulador notificadorInfoUsuarioSimulador) {
        ...
        colaCoordRobot = new LinkedBlockingDeque<>();
//        termina=false;

        visualizarPosicionRobot = new VisualizacionPosRobot();
        executor = Executors.newSingleThreadExecutor();
        visualizacionPosicionesIniciada = false;
...
//        this.visorMovimientoEscen.cambiarPosicionRobot(identRobot, coordX, coordY);
        ...
        if (!visualizacionPosicionesIniciada) {
            executor.submit(visualizarPosicionRobot);
            visualizacionPosicionesIniciada = true;
        }
        System.out.println(" El tamanyo de la cola es  :" +colaCoordRobot.size()+" El item encolado es :" + identRobot + " X : " + coordX + " Y : " + coordY);
        posRobot = new Object[3]; // almacenamos [1]ident robot, [2]coord X, [3]coord Y
        posRobot[0] = identRobot;
        posRobot[1] = coordX;
        posRobot[2] = coordY;
        colaCoordRobot.offer(posRobot);
    ...
    public void finCasoSimulacion() {
        ...
        visualizarPosicionRobot.termina();
        try {
...
//            this.setConsecuenciasNoSeleccionFicheroSimulacion();
 ...
 private class VisualizacionPosRobot implements Runnable {
            private static final long TIEMPO_ESPERA = 10;
            private boolean termina;
            public VisualizacionPosRobot() {
//			this.setDaemon(true);
			termina = false;
		}
            @Override
...
//					log.debug("Recogiendo item desde el buzon de items de la percepcion...");
                    System.out.println("En el thread " + Thread.currentThread().getName() + "Obteniendo items de la cola de coordenadas para visualizar ");
                    if (! colaCoordRobot.isEmpty()){
                    item = (Object[]) colaCoordRobot.take();
                    if (item != null) {
                        String idRobot = (String) item[0];
                        Integer x = (Integer) item[1];
                        Integer y = (Integer) item[2];
                        System.out.println("El item obtenido de la cola es :" + idRobot + " X : " + x + " Y : " + y);
                        visorMovimientoEscen.cambiarPosicionRobot(idRobot, x, y);
                        Thread.sleep(200);
                    }
//                   }                     TimeUnit.MILLISECONDS.sleep(400);
                System.out.println(" El item obtenido es null ");
                    } else //                                            log.debug("Item == NULL!!!!!");
                    {
                        System.out.println(" La cola de coordenadas esta vacia ");
                    }
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
